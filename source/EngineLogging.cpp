...
namespace Kiaro
{
    namespace Logging
    {
        ...
        Kiaro::Common::U32 Write(Kiaro::Support::String input, LEVEL_NAME log_level, Kiaro::Common::U32 arg_count, ...)
        {
            ...
            {
                PHYSFS_write(file, result_string, strlen(result_string), 1);
                PHYSFS_write(file, "\n", 1, 1);
                PHYSFS_close(file);
            }

            for (Kiaro::Common::U32 i = 0; i < kiaro_logging_hook_count; i++)
                kiaro_logging_hooks[i](result_string, log_level);
