...
namespace Kiaro
{
    namespace Phys
    {
        ...
        {
            this->drawLine(Common::Vector3DF(from.x(), from.y(), from.z()), Common::Vector3DF(to.x(), to.y(), to.z()), Common::ColorRGBA());
        }
        ...
        {
            this->drawContactPoint(Common::Vector3DF(pointOnB.x(), pointOnB.y(), pointOnB.z()), Common::Vector3DF(normalOnB.x(), normalOnB.y(), normalOnB.z()), distance, lifeTime, Common::ColorRGBA());
        }

        void IDebugRenderer::reportErrorWarning(const char* warningString)
        {
            CONSOLE_WARNINGF("Bullet: %s", warningString);
        }
