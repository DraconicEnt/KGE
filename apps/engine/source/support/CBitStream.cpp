...
namespace Kiaro
{
    namespace Support
    {
        CBitStream::CBitStream(ISerializable *in) : mTotalSize(in->getRequiredMemory()), mMemoryBlock(new unsigned char[in->getRequiredMemory()]), mPointer(0),
        mOwnsMemoryBlock(true)
        {
            in->writeTo(*this);
        }

        CBitStream::CBitStream(void *initializer, const size_t &initializerLength) : mMemoryBlock((unsigned char *)initializer), mTotalSize(initializerLength),
        ...
        : mMemoryBlock(new unsigned char[sizeInBytes]), mPointer(0), mTotalSize(sizeInBytes), mOwnsMemoryBlock(true)
        {
        ...
        template <>
        void CBitStream::write(char *string)
        {
            const StringLengthType stringLength = strlen(string);
            ...
            this->write<StringLengthType>(stringLength);
        }

        template <>
        char *CBitStream::extract(void)
        {
            // Read the string length off
            const size_t &stringLength = this->top<StringLengthType>();
            this->pop<StringLengthType>();

            // Set the pointer
            mPointer -= stringLength;

            // Allocate a new block with an extra byte for NULL termination
            char *newBlock = new char[stringLength + 1];
        ...
        template <>
        void CBitStream::pop<const char *>(void)
        {
            const size_t totalBytes = *((size_t*)&mMemoryBlock[mPointer]) + sizeof(StringLengthType);

            if (mPointer <= 0 || mTotalSize - mPointer < totalBytes)
        ...
        {
            unsigned char *newBlock = new unsigned char[newSize];
