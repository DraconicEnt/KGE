...
#include <engine/CBulletDebugDrawer.hpp>
...
namespace Kiaro
{
    namespace Network
    {
        static Kiaro::Network::SClient *OutgoingClientSingleton_Instance = NULL;

        SClient::SClient(irr::IrrlichtDevice *irrlicht, ENetPeer *incoming, Kiaro::Network::ServerBase *server) : mIsConnected(false), mPort(0), mCurrentStage(0), mInternalPeer(NULL), mInternalHost(NULL)
        {
            mEntityGroup = Kiaro::Engine::SGameWorld::getPointer();

            mBroadphase = new btDbvtBroadphase();
            mCollisionConfiguration = new btDefaultCollisionConfiguration();
            mPhysicalDebugger = new Kiaro::Engine::CBulletDebugDrawer(irrlicht);
        ...
        void SClient::onReceivePacket(Kiaro::Support::CBitStream &incomingStream)
        {
        ...
        SClient *SClient::getPointer(irr::IrrlichtDevice *irrlicht)
        {
            if (!OutgoingClientSingleton_Instance)
                OutgoingClientSingleton_Instance = new SClient(irrlicht, NULL, NULL);
        ...
        void SClient::send(Kiaro::Network::IMessage *packet, const bool &reliable)
        {
            ...
            Kiaro::Support::CBitStream outStream;
        ...
        void SClient::connect(const Kiaro::Common::String &targetAddress, const Kiaro::Common::U16 &targetPort, const Kiaro::Common::U32 &wait)
        {
			// TODO: Report Error
        ...
        void SClient::disconnect(const Kiaro::Common::U32 &waitTimeMS)
        {
            if (!mIsConnected)
                return;
